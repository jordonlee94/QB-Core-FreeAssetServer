...
RegisterNetEvent('qb-taxi:server:NpcPay', function(payment, hasReceivedBonus)
    ...
    if Player.PlayerData.job.name == Config.jobRequired then
        if NearTaxi(src) then
            ...
            if chance < 26 then
                exports['ps-inventory']:AddItem(src, 'cryptostick', 1, false, false, 'qb-taxi:server:NpcPay')
                TriggerClientEvent('ps-inventory:client:ItemBox', src, QBCore.Shared.Items['cryptostick'], 'add')
            end
        else
